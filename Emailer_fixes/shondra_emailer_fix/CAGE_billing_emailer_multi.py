...
def Emailer(text, signature, subject, recipient, recipient2, proj_list,scope, multi=False):
...
def original_emailer(bill_df, signature):
    for i in range(bill_df["SRM_no"].count()):
        ...
        #pass scope so shaina isnt added to designs
        Emailer(str(text), signature, subject, user, PI, proj_no,scope)
